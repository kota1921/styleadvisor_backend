...
- Комментарии: минимальные, не дублирующие код.
- Новое правило: комментарии в коде добавлять ТОЛЬКО по явному запросу пользователя; самостоятельно не вставлять пояснительные комментарии при обычной реализации/рефакторинге.
- ИСКЛЮЧЕНИЕ: для скриптов `deploy.sh` и `test_deploy.sh` допускаются развёрнутые комментарии (пошаговые пояснения и цели команд) ради прозрачности процесса деплоя.
...
- PR без зелёного статуса — не мерджится.
...
- Комментарии минимальные: не дублировать код, не «обмазывать» очевидное; отдаём приоритет выразительным именам и маленьким функциям.

### External Services Mock Policy
- Тесты не вызывают реальные API (OpenAI и т.п.).
- Использовать фикстуры / monkeypatch для изоляции.
